...
import 'package:gt_mediasoup/src/features/peers/enitity/peer.dart';
...
class PeersBloc extends Bloc<dynamic, PeersState> {
  ...
  PeersBloc({required this.mediaDevicesBloc}) : super(PeersState()) {
    ...
    on<PeerRemove>((event, emit) async {
      final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
      newPeers.remove(event.peerId);

      emit(PeersState(peers: newPeers));
    });
    on<PeerAddConsumer>((event, emit) async {
      final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
      ...
      if (kIsWeb) {
        ...
        if (event.consumer?.kind == 'data') {
          newPeers[event.peerId!] = newPeers[event.peerId]!.copyWith(dataConsumer: event.dataConsumer);
        }
      } else {
        if (event.consumer?.kind == 'video') {
    ...
    on<PeerRemoveConsumer>((event, emit) async {
      final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
      ...
      if (peer != null) {
        if (kIsWeb) {
          if (peer.audio?.id == event.consumerId) {
            ...
            await consumer?.close();
          } else if (peer.dataConsumer?.id == event.consumerId) {
            final consumer = peer.video;
            if (peer.dataConsumer != null) {
              newPeers[peer.id] = newPeers[peer.id]!.removeDataConsumer();
            }
            emit(PeersState(peers: newPeers));
            await consumer?.close();
          }
    ...
    on<PeerPausedConsumer>((event, emit) async {
      final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
      final Peer? peer = newPeers.values
          .firstWhereOrNull((p) => p.consumers.contains(event.consumerId));
    ...
    on<PeerResumedConsumer>((event, emit) async {
      final Map<String, Peer> newPeers = Map<String, Peer>.of(state.peers);
      final Peer? peer = newPeers.values
          .firstWhereOrNull((p) => p.consumers.contains(event.consumerId));
