...
class MediaDevicesBloc extends Bloc<MediaDevicesEvent, MediaDevicesState> {
  MediaDevicesBloc() : super(MediaDevicesState()) {
    on<MediaDeviceLoadDevices>((event, emit) async {
      try {
    ...
    on<MediaDeviceSelectAudioInput>((event, emit) async {
      emit(state.copyWith(
        selectedAudioInput: event.device,
      ));
    });
    on<MediaDeviceSelectAudioOutput>((event, emit) async {
      emit(state.copyWith(
        selectedAudioOutput: event.device,
      ));
    });
    on<MediaDeviceSelectVideoInput>((event, emit) async {
      emit(state.copyWith(
